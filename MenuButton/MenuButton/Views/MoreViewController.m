...
@interface MoreViewController ()<CustomGridDelegate,UIScrollViewDelegate>
{
    //标记是否选中
    BOOL isSelected;
    BOOL isSkip;
...
- (void)drawMoreGridView {
    ...
    [self.showMoreGridView removeFromSuperview];
    
    scrollView = [[UIScrollView alloc] initWithFrame:CGRectMake(0, 0, SCREEN_Width, SCREEN_Height-64)];
    scrollView.contentInset = UIEdgeInsetsMake(0, 0, SCREEN_Height*2, 0);
    scrollView.backgroundColor = [UIColor clearColor];
    scrollView.delegate = self;
    [self.view addSubview:scrollView];
    
    _showMoreGridView = [[UIView alloc] init];
    [_showMoreGridView setFrame:CGRectMake(0, 0, SCREEN_Width, GridHeight * PerColumGridCount)];
    ...
    [scrollView addSubview:_showMoreGridView];
    [self getNewData];
}
...
- (void)getNewData {
    ...
    scrollView.contentInset = UIEdgeInsetsMake(0, 0, gridHeight, 0);
}
...
-(void)saveArray
{
    ...
    NSInteger gridHeight;
    gridHeight = 123 * (self.showGridArray.count/3);
    scrollView.contentInset = UIEdgeInsetsMake(0, 0, gridHeight + 150, 0);
